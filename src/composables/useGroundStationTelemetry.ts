...
export type GroundStationTelemetryStatus =
  | 'empty'
  | 'good'
  | 'warning'
  | 'bad';
...
export function useGroundStationTelemetry(
  options: UseGroundStationTelemetryOptions
) {
  const {
    missionSeconds,
    simulationStatus,
...
const gs1GroundTelemetryAvailable =
  simulationStatus.value === 'RUNNING';

// Spacecraft-to-GS1 RF link availability
const tmVisible =
  gs1DownlinkAvailable.value &&
...
const ebno = !tmVisible
  ...
  : gs1Degrading
    ...
    const gs1AzEncoder =
      azimuth.value === null
        ? null
        : azimuth.value +
          Math.sin(missionSeconds.value / 7) * 0.012;

    const gs1ElEncoder =
      gs1DisplayedElevation.value === null
        ? null
        : gs1DisplayedElevation.value +
          Math.sin(missionSeconds.value / 8 + 0.8) * 0.010;

    const gs1AzError =
      gs1AzEncoder === null || azimuth.value === null
        ? 0
        : gs1AzEncoder - azimuth.value;
    ...
    const gs1PointingError =
      Math.sqrt(
        gs1AzError * gs1AzError +
        gs1ElError * gs1ElError
      );

    const gs1AzDriveTemp =
      oscillation(31.5, 1.2, 18, 0.4) +
      passProgress.value * 1.3;

    const gs1ElDriveTemp =
      oscillation(32.8, 1.3, 17, 1.1) +
      passProgress.value * 1.5;

    const gs1RfFeedTemp =
      oscillation(24.5, 1.0, 20, 1.7);

    const gs1DiplexerTemp =
      oscillation(30.5, 0.8, 22, 0.6);

    const gs1DuplexerTemp =
      oscillation(31.2, 0.8, 23, 1.4);

    const gs1WaveguideTemp =
      oscillation(22.8, 1.2, 24, 2.0);

    const gs1LnaGain =
      38.2 +
      Math.sin(missionSeconds.value / 20) * 0.15;
    ...
    const gs1Cn0 =
      ebno +
      10 * Math.log10(gs1TmDataRateKbps * 1000);
  ...
  gs1Degrading
    ? 2.5e-3 +
      Math.abs(
        Math.sin(missionSeconds.value / 5)
      ) * 1.5e-3
    : signalFiltered.value
      ? 1e-7 *
        Math.pow(10, -(ebno - 8.0) / 2)
      : 2e-4 *
        Math.pow(10, -(ebno - 5.0) / 1.5);
  ...
  gs1GeometryElevation.value === null
    ...
    : gs1MaxDopplerKHz *
      Math.cos(
        Math.PI *
        gs1GeometryProgress.value
      );

    const gs1ConfiguredFrequencyMHz =
      2250.000;

    const gs1ReceivedFrequencyMHz =
      gs1ConfiguredFrequencyMHz +
      gs1DopplerOffset / 1000;

    const gs1FrequencyReferenceError =
      Math.sin(missionSeconds.value / 29) * 0.035;

    const gs1TimeReferenceOffset =
      Math.sin(missionSeconds.value / 27 + 0.6) * 0.18;

    const gs1SpectrumNoiseFloor =
      -128.5 +
      Math.sin(missionSeconds.value / 18 + 1.3) * 0.7;

    const gs1BasebandLoad =
      signalFiltered.value
        ? oscillation(43, 3, 17, 0.4)
        : oscillation(57, 4, 16, 0.9);

    const gs1NetworkLatency =
      oscillation(11.5, 1.8, 19, 1.2);

    const gs1AmbientTemperature =
      oscillation(16.5, 1.2, 45, 0.8);

    const gs1Humidity =
      oscillation(44, 3, 50, 1.5);

    const gs1Pressure =
      oscillation(1013, 2.0, 60, 0.4);

    const gs1Precipitation = 0;

    const gs1EmergencyPowerSoc =
      oscillation(98.0, 0.3, 80, 0.7);
...
status: 'empty',
  ...
  measurement:
    tmVisible
      ? oscillation(-1.2, 0.4, 15, 1.2).toFixed(1)
      : 'NO SIGNAL',
  unit: 'kHz',
  status:
    tmVisible
...
},
{
  ...
  status:
    tmVisible
      ? (signalClass.value.replace('status-', '') as TelemetryStatus)
      : 'empty',
},
{
  parameter: 'GCL001',
...

{
  ...
  unit: '°C',
  status:
    gs1RfFeedTemp >= -10 && gs1RfFeedTemp <= 50
      ? 'good'
      : 'warning',
},
{
...
},
{
  ...
  unit: '°C',
  status:
    gs1WaveguideTemp >= -15 && gs1WaveguideTemp <= 50
      ? 'good'
      : 'warning',
},
{
...
},
{
  ...
  measurement:
    tmVisible
      ? gs1ReceivedFrequencyMHz.toFixed(6)
      : 'NO SIGNAL',
  unit: 'MHz',
  status:
    tmVisible
...
},
{
  ...
  unit: 'ppb',
  status:
    Math.abs(gs1FrequencyReferenceError) <= 0.1
      ? 'good'
      : 'warning',
},
{
  ...
  unit: 'µs',
  status:
    Math.abs(gs1TimeReferenceOffset) <= 1
      ? 'good'
      : 'warning',
},
...
},
{
  ...
  unit: 'ms',
  status:
    gs1NetworkLatency <= 30
      ? 'good'
      : gs1NetworkLatency <= 60
        ? 'warning'
        : 'bad',
},
{
...

{
  ...
  unit: '%',
  status:
    gs1EmergencyPowerSoc >= 80
      ? 'good'
      : gs1EmergencyPowerSoc >= 50
        ? 'warning'
        : 'bad',
},
{
  ...
  unit: '°C',
  status:
    gs1AmbientTemperature >= -10 &&
    gs1AmbientTemperature <= 40
      ? 'good'
      : 'warning',
},
{
...
},
{
  ...
  simulationStatus.value === 'RUNNING';
    ...
    const gs2AzEncoder =
      gs2Azimuth.value === null
        ? null
        : gs2Azimuth.value +
          Math.sin(missionSeconds.value / 7 + 0.9) * 0.014;

    const gs2ElEncoder =
      gs2Elevation.value === null
        ? null
        : gs2Elevation.value +
          Math.sin(missionSeconds.value / 8 + 1.5) * 0.011;

    const gs2AzError =
      gs2AzEncoder === null ||
      gs2Azimuth.value === null
        ? 0
        : gs2AzEncoder - gs2Azimuth.value;

    const gs2ElError =
      gs2ElEncoder === null ||
      gs2Elevation.value === null
        ? 0
        : gs2ElEncoder - gs2Elevation.value;

    const gs2PointingError =
      Math.sqrt(
        gs2AzError * gs2AzError +
        gs2ElError * gs2ElError
      );

    const gs2AzDriveTemp =
      oscillation(30.2, 1.1, 19, 1.0);

    const gs2ElDriveTemp =
      oscillation(31.4, 1.2, 18, 1.7);

    const gs2RfFeedTemp =
      oscillation(22.8, 1.0, 21, 2.0);

    const gs2DiplexerTemp =
      oscillation(29.3, 0.8, 23, 1.0);

    const gs2DuplexerTemp =
      oscillation(30.0, 0.8, 24, 1.7);

    const gs2WaveguideTemp =
      oscillation(21.0, 1.0, 25, 2.4);

    const gs2LnaGain =
      39.0 +
      Math.sin(missionSeconds.value / 21 + 0.5) * 0.15;
    ...
    const gs2Cn0 =
      trackingStarted
        ? ebno +
          10 * Math.log10(gs2TmDataRateKbps * 1000)
        : 0;
  ...
  gs2Elevation.value !== null
    ...
    const gs2ConfiguredFrequencyMHz =
      2250.000;

    const gs2ReceivedFrequencyMHz =
      gs2ConfiguredFrequencyMHz +
      gs2DopplerOffset / 1000;

    const gs2FrequencyReferenceError =
      Math.sin(missionSeconds.value / 31 + 0.6) * 0.030;

    const gs2TimeReferenceOffset =
      Math.sin(missionSeconds.value / 29 + 1.0) * 0.15;

    const gs2SpectrumNoiseFloor =
      -129.0 +
      Math.sin(missionSeconds.value / 20 + 1.8) * 0.6;

    const gs2BasebandLoad =
      !trackingStarted
    ...
    const gs2NetworkLatency =
      oscillation(13.0, 2.0, 21, 1.5);

    const gs2AmbientTemperature =
      oscillation(13.8, 1.3, 48, 1.2);

    const gs2Humidity =
      oscillation(48, 3, 53, 2.0);

    const gs2Pressure =
      oscillation(1009, 2.2, 64, 0.8);

    const gs2Precipitation = 0;

    const gs2EmergencyPowerSoc =
      oscillation(97.5, 0.3, 85, 1.0);
  ...
  subsystem: 'Antenna Azimuth',
  measurement:
    trackingStarted && gs2Azimuth.value !== null
      ? gs2Azimuth.value.toFixed(1)
      : 'NO DATA',
  unit: 'deg',
  status:
    trackingStarted && gs2Azimuth.value !== null
  ...
  subsystem: 'Slant Range',
  measurement:
    trackingStarted && gs2Range.value !== null
      ? gs2Range.value.toFixed(0)
      : 'NO DATA',
  unit: 'km',
  status:
    trackingStarted && gs2Range.value !== null
...
},
{
  ...
  subsystem: 'Azimuth Position Encoder',
  measurement:
    !trackingStarted || gs2AzEncoder === null
      ? 'NO DATA'
      : gs2AzEncoder.toFixed(2),
  unit: 'deg',
  status:
    !trackingStarted || gs2AzEncoder === null
      ? 'empty'
...
},
{
  ...
  subsystem: 'Elevation Position Encoder',
  measurement:
    !trackingStarted || gs2ElEncoder === null
      ? 'NO DATA'
      : gs2ElEncoder.toFixed(2),
  unit: 'deg',
  status:
    !trackingStarted || gs2ElEncoder === null
      ? 'empty'
...
},
{
  ...
  measurement:
    gs2Online
      ? gs2ConfiguredFrequencyMHz.toFixed(3)
      : 'NO DATA',
  unit: 'MHz',
  status: gs2Online ? 'good' : 'empty',
},
{
  ...
  measurement:
    linkReady
      ? gs2ReceivedFrequencyMHz.toFixed(6)
      : 'NO SIGNAL',
  unit: 'MHz',
...
},
{
  ...
  measurement:
    gs2Online
      ? gs2FrequencyReferenceError.toFixed(3)
      : 'NO DATA',
  unit: 'ppb',
  status:
    !gs2Online
...
},
{
  ...
  measurement:
    gs2Online
      ? gs2TimeReferenceOffset.toFixed(3)
      : 'NO DATA',
  unit: 'µs',
  status:
    !gs2Online
      ? 'empty'
      : Math.abs(gs2TimeReferenceOffset) <= 1
        ? 'good'
        : 'warning',
},
...

{
  ...
  measurement:
    gs2Online
      ? gs2SpectrumNoiseFloor.toFixed(1)
      : 'NO DATA',
  unit: 'dBm',
  status: gs2Online ? 'good' : 'empty',
},
...

{
  ...
  measurement:
    gs2Online
      ? gs2EmergencyPowerSoc.toFixed(1)
      : 'NO DATA',
  unit: '%',
  status:
    !gs2Online
...
},
{
  ...
  measurement:
    gs2Online
      ? gs2AmbientTemperature.toFixed(1)
      : 'NO DATA',
  unit: '°C',
  status: gs2Online ? 'good' : 'empty',
},
