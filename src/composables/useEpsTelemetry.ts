...
export type EpsTelemetryStatus =
  | 'empty'
  | 'good'
  | 'warning'
  | 'bad';
...
export function useEpsTelemetry(
  options: UseEpsTelemetryOptions
) {
  const {
    missionSeconds,
...
const epsTelemetryDefinition = [
  ...
  const payloadLoadFactor = clamp(
    (payloadPowerLevel.value - 8) / 172,
    0,
    1
  );
  ...
  const thermalStress = Math.max(
    0,
    epsTemperature.value - 75
  );

  const batteryTransferActive =
    isScenario2.value &&
  ...
  const payloadBusCurrent =
    payloadPowerLevel.value /
    Math.max(payloadBusVoltage, 1);

  const essentialBusCurrent =
    7.2 +
    Math.sin(missionSeconds.value / 12) * 0.12;

  const nonessentialBusCurrent = clamp(
    3.6 -
      powerSavingFactor * 2.3 +
      Math.sin(missionSeconds.value / 14 + 0.6) * 0.15,
    1.0,
    4.0
  );

  const mainBusCurrent =
    essentialBusCurrent +
    nonessentialBusCurrent +
    payloadBusCurrent +
    0.7;

  const mainBusPower =
    mainBusVoltage * mainBusCurrent;

  const essentialBusVoltage = clamp(
    mainBusVoltage -
      0.04 +
      Math.sin(missionSeconds.value / 11) * 0.015,
    27.5,
    28.8
  );
  ...
  const nonessentialBusVoltage = clamp(
    mainBusVoltage -
      0.09 -
      payloadLoadFactor * 0.02 +
      Math.sin(missionSeconds.value / 13) * 0.02,
    27.2,
    28.7
  );
  ...
  const generatedSolarPower =
    mainBusPower + netPower.value;

  const solarArrayVoltage = clamp(
    196 +
      Math.sin(missionSeconds.value / 17 + 0.4) * 2.6,
    188,
    204
  );

  const solarArrayCurrent =
    generatedSolarPower /
    solarArrayVoltage;

  const solarArrayTempX =
    -7.0 +
    Math.sin(missionSeconds.value / 20 + 2.2) * 4.8;

  const solarArrayTempY =
    -8.2 +
    Math.sin(missionSeconds.value / 22 + 1.1) * 4.5;

  const solarHarnessTemp =
    11.5 +
    payloadLoadFactor * 1.2 +
    Math.sin(missionSeconds.value / 18) * 1.1;

  const bypassDiodeTemp =
    31.0 +
    solarArrayCurrent * 0.65 +
    Math.sin(missionSeconds.value / 15) * 0.7;

  const blockingDiodeTemp =
    32.0 +
    solarArrayCurrent * 0.70 +
    Math.sin(missionSeconds.value / 16 + 0.8) * 0.7;

  const sadeTemp =
    35.5 +
    solarArrayCurrent * 0.25 +
    Math.sin(missionSeconds.value / 15 + 1.2) * 0.6;

  const mpptEfficiency = clamp(
    96.8 -
      payloadLoadFactor * 0.20 +
      Math.sin(missionSeconds.value / 24) * 0.15,
    95.5,
    97.3
  );
  ...
  const cellVoltageFromSoc = (soc: number) =>
    clamp(
      3.05 + (soc / 100) * 1.15,
      3.05,
      4.20
  ...
  const batteryAVoltage =
    cellVoltageA * 8;

  const batteryBVoltage =
    cellVoltageB * 8;

  const batteryCVoltage =
    cellVoltageC * 8;
  ...
  if (batteryTransferActive) {
    ...
    batteryACurrent =
      -4.0 +
      Math.sin(missionSeconds.value / 6) * 0.08;

    batteryBCurrent =
      2.0 +
      Math.sin(missionSeconds.value / 7 + 0.5) * 0.06;

    batteryCCurrent =
      2.0 +
      Math.sin(missionSeconds.value / 7 + 1.1) * 0.06;
  } else {
    const normalDischargeCurrent =
      -0.60 -
      payloadLoadFactor * 0.80;

    batteryACurrent =
      normalDischargeCurrent +
      Math.sin(missionSeconds.value / 13) * 0.06;

    batteryBCurrent =
      normalDischargeCurrent +
      Math.sin(missionSeconds.value / 14 + 0.7) * 0.06;

    batteryCCurrent =
      normalDischargeCurrent +
      Math.sin(missionSeconds.value / 15 + 1.4) * 0.06;
  }

  const averageBatteryCurrent =
    (
      Math.abs(batteryACurrent) +
      Math.abs(batteryBCurrent) +
      Math.abs(batteryCCurrent)
    ) / 3;
  ...
  const cellSpread = (
    ...
    clamp(
      0.018 +
        (100 - soc) * 0.00015 +
        Math.sin(missionSeconds.value / 30 + phase) * 0.0015,
      0.012,
      0.045
    );
  ...
  const cellAMin =
    cellVoltageA - spreadA / 2;

  const cellAMax =
    cellVoltageA + spreadA / 2;

  const cellBMin =
    cellVoltageB - spreadB / 2;

  const cellBMax =
    cellVoltageB + spreadB / 2;

  const cellCMin =
    cellVoltageC - spreadC / 2;

  const cellCMax =
    cellVoltageC + spreadC / 2;

  const cellTempSpread =
    0.45 +
    averageBatteryCurrent * 0.04;

  const batteryACellMinTemp =
    batteryTemp - cellTempSpread;

  const batteryACellMaxTemp =
    batteryTemp + cellTempSpread;

  const batteryBCellMinTemp =
    batteryTemp - cellTempSpread + 0.2;

  const batteryBCellMaxTemp =
    batteryTemp + cellTempSpread + 0.2;

  const batteryCCellMinTemp =
    batteryTemp - cellTempSpread - 0.1;

  const batteryCCellMaxTemp =
    batteryTemp + cellTempSpread - 0.1;

  const batteryOperatingMode =
    batteryTransferActive
      ? 'TRANSFER'
      : averageSoc < 20
        ? 'RECOVERY REQUIRED'
        : 'DISCHARGE';
  ...
  const pcuEfficiency = clamp(
    95.6 -
      payloadLoadFactor * 0.25 +
      Math.sin(missionSeconds.value / 25) * 0.12,
    94.5,
    96.5
  );
  ...
  const converterEfficiency = clamp(
    94.4 -
      payloadLoadFactor * 0.35 +
      Math.sin(missionSeconds.value / 22 + 0.4) * 0.15,
    93.0,
    95.2
  );
  ...
  const voltageRegulatorOutput = clamp(
    mainBusVoltage +
      Math.sin(missionSeconds.value / 20) * 0.01,
    27.6,
    28.8
  );
  ...
  const lclTemp =
    ...
    Math.sin(missionSeconds.value / 17 + 1.7) * 0.5;

  const pduChannel4Load = clamp(
    15 +
      payloadLoadFactor * 69 +
      Math.sin(missionSeconds.value / 15) * 1.5,
    10,
    88
  );
  ...
  const peakPowerDemand =
    mainBusPower +
    75 +
    payloadLoadFactor * 15;
  ...
  return [
...

    {
      ...
      unit: '°C',
      status:
        epsTemperatureClass.value.replace(
          'status-',
          ''
        ) as TelemetryStatus,
    },
...

    {
      ...
      unit: 'state',
      status:
        epsMode === 'EPS_DEGRADED'
          ? 'bad'
          : epsMode === 'EPS_LOAD_SHEDDING'
            ? 'warning'
            : 'good',
    },
