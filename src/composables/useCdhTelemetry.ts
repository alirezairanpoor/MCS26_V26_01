...
export type CdhTelemetryStatus = 'empty' | 'good' | 'warning' | 'bad';
...
function noTelemetryRows(definitions: TelemetryDefinition[]): CdhTelemetryRow[] {
...
function percentStatus(value: number, warningAt: number, badAt: number): CdhTelemetryStatus {
  if (value >= badAt) {
    return 'bad';
  }
...
function formatOnboardTime(totalSeconds: number) {
...
export function useCdhTelemetry(options: UseCdhTelemetryOptions) {
  const {
    missionSeconds,
  ...
  const cdhTelemetry = computed<CdhTelemetryRow[]>(() => {
    ...
    if (!spacecraftTelemetryAvailable.value || scenario2TelemetryBlackout.value) {
      return noTelemetryRows(cdhTelemetryDefinition);
    }

    const t = missionSeconds.value;

    const massMemoryUsed = clamp(memoryUsed.value, 0, 100);

    const massMemoryAvailable = 100 - massMemoryUsed;

    const dumpActive = memoryDumpStarted.value && !memoryDumpComplete.value;

    const dumpFinished = memoryDumpComplete.value;

    // ----------------------------------------------------------------------
    // COMMAND HISTORY - derived from actual SPACON GO history.
    // ----------------------------------------------------------------------

    const commandHistory = tcHistory.value;

    const commandAcceptedCount = commandHistory.filter(
      (entry) => !entry.result.startsWith('FAILED')
    ).length;

    const commandRejectedCount = commandHistory.filter((entry) =>
      entry.result.startsWith('FAILED')
    ).length;

    const lastCommand = commandHistory.length > 0 ? commandHistory[0] : null;

    // ----------------------------------------------------------------------
    // PROCESSING
    // ----------------------------------------------------------------------

    const processorLoad = clamp(
      32 + (dumpActive ? 12 : 0) + (imageTaken.value ? 4 : 0) + wave(t, 0, 5, 17, 0.4),
      15,
      85
    );

    const processorIdle = clamp(100 - processorLoad, 0, 100);

    const processorTemp = 38 + processorLoad * 0.1 + wave(t, 0, 0.8, 31, 0.5);

    const fpgaLoad = clamp(24 + (dumpActive ? 18 : 0) + wave(t, 0, 4, 19, 0.9), 10, 85);

    const fpgaTemp = 39 + fpgaLoad * 0.09 + wave(t, 0, 0.7, 34, 1.0);

    const watchdogKickAge = Math.round(clamp(wave(t, 45, 12, 13, 0.4), 10, 100));

    // ----------------------------------------------------------------------
    // MEMORY
    // ----------------------------------------------------------------------

    const memoryControllerTemp = 39.0 + (dumpActive ? 2.5 : 0) + wave(t, 0, 1.3, 13, 0.3);

    const ssdTemp = 42.0 + (dumpActive ? 3.0 : 0) + wave(t, 0, 1.5, 15, 1.1);

    const externalBayTemp = wave(t, -4.0, 4.0, 20, 2.7);

    const memoryIoFpgaTemp = 44.0 + (dumpActive ? 2.0 : 0) + wave(t, 0, 1.2, 14, 0.8);

    const sdramUse = clamp(
      38 + processorLoad * 0.2 + (dumpActive ? 8 : 0) + wave(t, 0, 3, 21, 0.6),
      20,
      82
    );

    const packetBufferUse = dumpActive
      ? clamp(massMemoryUsed - 12, 4, 85)
      : clamp(18 + massMemoryUsed * 0.18 + wave(t, 0, 3, 20, 0.7), 5, 70);

    const rawImagePartition = clamp(massMemoryUsed + 6, 0, 98);

    const housekeepingPartition = clamp(wave(t, 32, 2.0, 17, 0.9), 20, 45);

    const fileIndexUse = clamp(45 + massMemoryUsed * 0.22 + wave(t, 0, 2, 21, 0.4), 35, 75);

    const dumpProgress =
      dumpActive || dumpFinished ? clamp(100 - (massMemoryUsed / 87) * 100, 0, 100) : 0;

    const dumpPointer = dumpActive || dumpFinished ? dumpProgress : 0;

    const eccCorrectedCount = Math.max(0, Math.floor(2 + Math.sin(t / 23) * 2));

    const badBlockCount = 1;

    const downlinkQueueDepth = dumpActive
      ? clamp(massMemoryUsed * 0.75, 5, 95)
      : clamp(massMemoryUsed * 0.08, 0, 20);

    // ----------------------------------------------------------------------
    // NETWORK
    // ----------------------------------------------------------------------

    const spaceWireUtilization = clamp(
      24 + (dumpActive ? 28 : 0) + (imageTaken.value ? 4 : 0) + wave(t, 0, 4, 18, 0.5),
      10,
      85
    );

    const canUtilization = clamp(wave(t, 18, 3, 22, 0.8), 8, 35);

    const spaceWireErrors = Math.max(0, Math.floor(0.6 + Math.sin(t / 71) * 0.6));

    const canErrors = 0;

    const busErrorCount = spaceWireErrors + canErrors;

    // ----------------------------------------------------------------------
    // TELEMETRY / PACKETS
    // ----------------------------------------------------------------------

    const telemetryRate = 8.0 + wave(t, 0, 0.25, 30, 0.4);

    const telemetryPacketCount = Math.max(0, Math.floor(t * telemetryRate));

    const packetErrorCount = Math.max(0, Math.floor(0.8 + Math.sin(t / 83) * 0.8));

    const telemetryBuffer = clamp(12 + (dumpActive ? 9 : 0) + wave(t, 0, 2, 19, 1.1), 5, 40);

    // ----------------------------------------------------------------------
    // SOFTWARE / TIME
    // ----------------------------------------------------------------------

    const scrubberProgress = (t * 0.35) % 100;

    const timeSyncError = wave(t, 0, 0.25, 37, 0.8);

    const clockDrift = wave(t, 0.12, 0.035, 55, 0.5);

    const openFileCount = 5 + (imageTaken.value ? 1 : 0) + (dumpActive ? 2 : 0);

    const directoryUtilization = clamp(28 + massMemoryUsed * 0.18, 20, 55);

    // ----------------------------------------------------------------------
    // HEALTH / STATUS
    // ----------------------------------------------------------------------

    const memoryStatus = massMemoryUsed <= 10 ? 'good' : massMemoryUsed <= 70 ? 'warning' : 'bad';
    ...
    const cdhOverallStatus: CdhTelemetryStatus =
      massMemoryUsed >= 95 ? 'bad' : massMemoryUsed > 70 ? 'warning' : 'good';
    ...
    return [
      // PROCESSING
      row('CDH001', 'C&DH Overall Health', cdhOverallHealth, 'state', cdhOverallStatus),

      row(
        'CDH002',
        'C&DH Operating Mode',
        dumpActive ? 'DATA DUMP SUPPORT' : 'NOMINAL OPERATIONS',
        'state',
        dumpActive ? 'warning' : 'good'
      ),

      row('OBC101', 'Onboard Computer A', 'ACTIVE', 'state'),

      row('OBC102', 'Onboard Computer B', 'HOT STANDBY', 'state'),

      row('OBC103', 'Active Onboard Computer', 'OBC-A', 'state'),

      row('CPU110', 'Central Processor State', 'RUNNING', 'state'),
      ...
      row(
        ...
        '%',
        percentStatus(processorLoad, 75, 90)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(processorTemp, 10, 55, -5, 70)
      ),

      row('CPU113', 'Processor Clock', '400.0', 'MHz'),

      row('CPU114', 'Processor Idle Time', processorIdle.toFixed(0), '%'),

      row('FPGA120', 'C&DH FPGA State', 'ACTIVE', 'state'),

      row('FPGA121', 'C&DH FPGA Load', fpgaLoad.toFixed(0), '%', percentStatus(fpgaLoad, 75, 90)),
      ...
      row(
        ...
        fpgaTemp.toFixed(1),
        '°C',
        temperatureStatus(fpgaTemp, 10, 58, -5, 70)
      ),

      row('RTU130', 'Remote Terminal Unit', 'ACTIVE', 'state'),

      row('IO131', 'Input/Output Module', 'ACTIVE', 'state'),

      row('IO132', 'Discrete I/O Channels Active', '24', 'count'),

      row('IO133', 'Analog Acquisition Channels Active', '16', 'count'),

      row('WDG140', 'Watchdog Status', 'HEALTHY', 'state'),
      ...
      row(
        ...
        'ms',
        watchdogKickAge <= 250 ? 'good' : 'warning'
      ),

      row('RST150', 'Reset Controller', 'ARMED', 'state'),

      row('RST151', 'Reset Cause', 'POWER-ON RESET', 'state'),

      row('RST152', 'Boot Count', '1', 'count'),

      // MEMORY
      row('MEM001', 'Memory Subsystem Health', memoryHealth, 'state', cdhOverallStatus),

      row('MEM010', 'Boot ROM', 'VALID / READ ONLY', 'state'),

      row('MEM020', 'EEPROM', 'VALID', 'state'),

      row('MEM030', 'MRAM', 'VALID', 'state'),

      row('MEM040', 'NAND Flash', 'ONLINE', 'state'),

      row('MEM050', 'SDRAM', 'ONLINE', 'state'),

      row('MEM051', 'SDRAM Utilization', sdramUse.toFixed(0), '%', percentStatus(sdramUse, 75, 90)),

      row('MEM052', 'SDRAM ECC Status', 'CORRECTING / NOMINAL', 'state'),

      row('MEM221', 'Payload Memory Used', massMemoryUsed.toFixed(0), '%', memoryStatus),
      ...
      row(
        ...
        '%',
        massMemoryAvailable >= 30 ? 'good' : massMemoryAvailable >= 10 ? 'warning' : 'bad'
      ),

      row(
        'MEM223',
        'Mass Memory State',
        dumpActive ? 'DUMP IN PROGRESS' : dumpFinished ? 'DUMP COMPLETE / READY' : 'ONLINE',
        'state',
        dumpActive ? 'warning' : 'good'
      ),

      row('MEM224', 'Payload Data Recorder', imageTaken.value ? 'DATA STORED' : 'READY', 'state'),
      ...
      row(
        ...
        '°C',
        temperatureStatus(memoryControllerTemp, 25, 48, 10, 60)
      ),
      ...
      row(
        ...
        ssdTemp.toFixed(1),
        '°C',
        temperatureStatus(ssdTemp, 28, 50, 10, 62)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(externalBayTemp, -18, 15, -35, 35)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(memoryIoFpgaTemp, 20, 55, 5, 68)
      ),
      ...
      row(
        ...
        '%',
        percentStatus(packetBufferUse, 70, 90)
      ),
      ...
      row(
        ...
        '%',
        percentStatus(rawImagePartition, 70, 90)
      ),

      row('MEM073', 'Housekeeping Partition', housekeepingPartition.toFixed(0), '%', 'good'),
      ...
      row(
        ...
        '%',
        dumpActive ? 'warning' : dumpFinished ? 'good' : 'empty'
      ),

      row('MEM009', 'ECC Corrected Counter', eccCorrectedCount.toString(), 'count', 'good'),

      row('MEM008', 'ECC Uncorrected Counter', '0', 'count', 'good'),

      row('MEM007', 'NAND Bad Block Count', badBlockCount.toString(), 'count', 'good'),

      row(
        'MEM806',
        'Downlink Queue',
        dumpActive ? 'ACTIVE' : 'STANDBY',
        'state',
        dumpActive ? 'warning' : 'good'
      ),
      ...
      row(
        ...
        '%',
        percentStatus(downlinkQueueDepth, 70, 90)
      ),

      row('MEM901', 'MMU Sync', 'SYNC', 'state'),

      row('MEM902', 'Checksum', 'OK', 'state'),

      // ONBOARD NETWORK
      row('NET201', 'Onboard Network Health', 'NOMINAL', 'state'),

      row('SPW210', 'SpaceWire Router', 'ACTIVE', 'state'),

      row('SPW211', 'SpaceWire Active Links', '6', 'count'),
      ...
      row(
        ...
        '%',
        percentStatus(spaceWireUtilization, 75, 90)
      ),
      ...
      row(
        ...
        'count',
        spaceWireErrors <= 2 ? 'good' : 'warning'
      ),

      row('CAN220', 'CAN Controller', 'ACTIVE', 'state'),

      row('CAN221', 'CAN Bus Utilization', canUtilization.toFixed(0), '%'),

      row('CAN222', 'CAN Bus Error Count', canErrors.toString(), 'count'),

      row('MIL230', 'MIL-STD-1553 Controller', 'NOT CONFIGURED', 'state', 'empty'),

      row('ETH240', 'Ethernet Switch', 'NOT CONFIGURED', 'state', 'empty'),

      row('UART250', 'UART Interface', 'AVAILABLE', 'state'),

      row('RS422251', 'RS-422 Interface', 'ACTIVE', 'state'),

      row('DIO260', 'Discrete Input/Output Interface', 'ACTIVE', 'state'),

      row('AAU270', 'Analog Acquisition Unit', 'ACTIVE', 'state'),
      ...
      row(
        ...
        'count',
        busErrorCount <= 2 ? 'good' : 'warning'
      ),

      // COMMAND HANDLING
      row('CMD301', 'Command Dispatcher', 'READY', 'state'),

      row('CMD302', 'Command Queue Depth', '0', 'count'),

      row('CMD303', 'Command Acceptance Count', commandAcceptedCount.toString(), 'count'),
      ...
      row(
        ...
        'count',
        commandRejectedCount === 0 ? 'good' : 'warning'
      ),

      row(
        'CMD305',
        'Last Executed Command',
        lastCommand ? lastCommand.command : 'NONE',
        'state',
        lastCommand ? 'good' : 'empty'
      ),

      row(
        'CMD306',
        'Last Command Result',
        lastCommand ? lastCommand.result : 'NONE',
        'state',
        lastCommand ? (lastCommand.result.startsWith('FAILED') ? 'bad' : 'good') : 'empty'
      ),

      row('CMD307', 'Command Decoder', 'VALID', 'state'),

      row('CMD308', 'Command Authentication', 'VALID', 'state'),

      // TELEMETRY / PACKETS
      row('TM401', 'Telemetry Manager', 'ACTIVE', 'state'),

      row('TM402', 'Packet Router', 'ACTIVE', 'state'),

      row('TM403', 'Telemetry Packet Count', telemetryPacketCount.toString(), 'count'),
      ...
      row(
        ...
        'count',
        packetErrorCount <= 2 ? 'good' : 'warning'
      ),

      row('TM405', 'Telemetry Generation Rate', telemetryRate.toFixed(2), 'pkt/s'),
      ...
      row(
        ...
        '%',
        percentStatus(telemetryBuffer, 70, 90)
      ),

      row('TM407', 'Housekeeping Packet Service', 'ACTIVE', 'state'),

      row('TM408', 'Event Packet Service', 'ACTIVE', 'state'),

      // SOFTWARE SERVICES
      row('SW501', 'Bootloader', 'VALID', 'state'),

      row('SW502', 'Real-Time Operating System', 'RUNNING', 'state'),

      row('SW503', 'Software Version', 'SIM-FSW-1.0', 'state'),

      row('SW504', 'Active Software Bank', 'BANK-A', 'state'),

      row('SW505', 'Task Status', 'ALL CRITICAL TASKS RUNNING', 'state'),

      row('SW506', 'Task Restart Count', '0', 'count'),

      row('SW510', 'Onboard Scheduler', 'ACTIVE', 'state'),

      row('SW511', 'Time Manager', 'LOCKED', 'state'),

      row('SW512', 'Mode Manager', 'ACTIVE', 'state'),

      row('SW513', 'FDIR Manager', 'ACTIVE', 'state'),

      row('SW514', 'Event Manager', 'ACTIVE', 'state'),

      row('SW520', 'File System', dumpActive ? 'MOUNTED / READ ACTIVE' : 'MOUNTED / RW', 'state'),

      row('SW521', 'Memory Scrubber', 'ACTIVE', 'state'),

      row('SW522', 'Memory Scrubber Progress', scrubberProgress.toFixed(1), '%'),

      row('SW530', 'Software Image Manager', 'NOMINAL', 'state'),

      row('SW531', 'Onboard Procedure Engine', 'AVAILABLE', 'state'),

      row('SW532', 'Security Manager', 'ACTIVE', 'state'),

      // TIME
      row('TIM601', 'Onboard Time', formatOnboardTime(t), 'hh:mm:ss'),

      row('TIM602', 'Time Synchronization', 'SYNCHRONIZED', 'state'),
      ...
      row(
        ...
        'ms',
        Math.abs(timeSyncError) <= 1 ? 'good' : 'warning'
      ),

      row('TIM604', '1 PPS Validity', 'VALID', 'state'),
      ...
      row(
        ...
        'ppm',
        Math.abs(clockDrift) <= 1 ? 'good' : 'warning'
      ),

      // FILE SYSTEM
      row('FIL701', 'File-System Status', 'MOUNTED / HEALTHY', 'state'),

      row('FIL702', 'Open File Count', openFileCount.toString(), 'count'),
      ...
      row(
        ...
        '%',
        percentStatus(directoryUtilization, 75, 90)
      ),

      row('FIL704', 'File-System Error Count', '0', 'count'),

      row(
        'FIL705',
        'Payload Image File State',
        imageTaken.value ? 'IMAGE FILE PRESENT' : 'NO NEW IMAGE FILE',
        'state',
      ...
      row(
        'DMP801',
        'Memory Dump State',
        dumpActive ? 'IN PROGRESS' : dumpFinished ? 'COMPLETE' : 'IDLE',
        'state',
        dumpActive ? 'warning' : 'good'
      ),
      ...
      row(
        ...
        dumpActive ? wave(t, 42, 4, 17, 0.5).toFixed(1) : '0.0',
        'MB/s',
        dumpActive ? 'good' : 'empty'
      ),

      row(
        'DMP804',
        'Memory Dump Data Integrity',
        dumpActive || dumpFinished ? 'VALID' : 'IDLE',
        'state',
        dumpActive || dumpFinished ? 'good' : 'empty'
      ),
    ];
  });
