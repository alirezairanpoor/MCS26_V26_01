...
export type TcsTelemetryStatus = 'empty' | 'good' | 'warning' | 'bad';
...
function noTelemetryRows(definitions: TelemetryDefinition[]): TcsTelemetryRow[] {
...
export function useTcsTelemetry(options: UseTcsTelemetryOptions) {
  const {
    missionSeconds,
  ...
  const tcsTelemetry = computed<TcsTelemetryRow[]>(() => {
    // No spacecraft telemetry path = no TCS telemetry visible on ground.
    if (!spacecraftTelemetryAvailable.value) {
      return noTelemetryRows(tcsTelemetryDefinition);
    }

    const t = missionSeconds.value;

    const epsTemp = epsTemperature.value;

    const payloadLoadFactor = clamp((payloadPowerLevel.value - 10) / 170, 0, 1);

    const thermalStress = Math.max(0, epsTemp - 75);

    const coolingActive = thermalCoolingActive.value;

    const batteryTransfer = batteryEqualizationInProgress.value;

    const payloadHighPower = payloadPowerRaised.value || powerIncreaseInProgress.value;

    const cameraConfiguredFactor = cameraConfigured.value ? 1 : 0;

    const standby = spacecraftStandbyActive.value;

    // ----------------------------------------------------------------------
    // TCS MODE
    // ----------------------------------------------------------------------

    const tcsMode = coolingActive
    ...
    const overallHealthStatus: TcsTelemetryStatus =
      epsTemp > 95 ? 'bad' : epsTemp > 80 ? 'warning' : 'good';
    ...
    const controllerTemp = 34.5 + thermalStress * 0.18 + wave(t, 0, 0.8, 31, 0.4);

    const controllerLoad = clamp(
      36 + thermalStress * 0.8 + (coolingActive ? 8 : 0) + wave(t, 0, 3, 22, 0.8),
      20,
      85
    );
    ...
    const pcuTemp =
      40.5 + thermalStress * 1.75 + payloadLoadFactor * 2.0 + wave(t, 0, 0.7, 19, 0.4);

    const dcDcTemp =
      43.0 + thermalStress * 1.55 + payloadLoadFactor * 3.2 + wave(t, 0, 0.8, 21, 0.9);

    const pduTemp =
      37.5 + thermalStress * 0.75 + payloadLoadFactor * 1.8 + wave(t, 0, 0.7, 23, 1.1);
    ...
    const batteryA = batteryBaseTemp + wave(t, 0, 0.35, 28, 0.2);

    const batteryB = batteryBaseTemp + 0.25 + wave(t, 0, 0.35, 30, 0.8);

    const batteryC = batteryBaseTemp - 0.15 + wave(t, 0, 0.35, 32, 1.4);

    const batteryAverage = (batteryA + batteryB + batteryC) / 3;

    const bmsTemp =
      30.5 + (batteryTransfer ? 1.8 : 0) + payloadLoadFactor * 0.5 + wave(t, 0, 0.45, 26, 0.5);

    const obcTemp = 36.5 + thermalStress * 0.2 + wave(t, 0, 0.9, 37, 0.4);

    const memoryTemp = 33.0 + payloadLoadFactor * 1.2 + wave(t, 0, 0.8, 41, 0.8);

    const commsTemp = 35.0 + thermalStress * 0.25 + wave(t, 0, 1.0, 34, 1.0);

    const transmitterTemp = 41.0 + thermalStress * 0.35 + wave(t, 0, 1.2, 29, 0.7);

    const aocsComputerTemp = 35.5 + thermalStress * 0.15 + wave(t, 0, 0.8, 43, 1.1);

    const reactionWheelTemp = 33.0 + wave(t, 0, 1.7, 46, 0.7);

    const starTrackerTemp = 22.5 + wave(t, 0, 1.3, 52, 1.2);
    ...
    const orbitalThermalWave = Math.sin(t / 95);

    const panelPlusX = 13 + orbitalThermalWave * 8 + wave(t, 0, 0.8, 27);

    const panelMinusX = 7 - orbitalThermalWave * 7 + wave(t, 0, 0.7, 31, 0.5);

    const panelPlusY = 11 + Math.sin(t / 105 + 0.8) * 7;

    const panelMinusY = 8 + Math.sin(t / 110 + 1.6) * 6;

    const panelPlusZ = 15 + Math.sin(t / 100 + 2.0) * 8;

    const panelMinusZ = 5 + Math.sin(t / 115 + 2.7) * 6;

    const opticalBenchTemp = 21.5 + payloadLoadFactor * 2.0 + wave(t, 0, 0.35, 60, 0.8);

    const avionicsDeckTemp = 29.0 + thermalStress * 0.22 + wave(t, 0, 0.6, 49, 0.5);

    // ----------------------------------------------------------------------
    // RADIATORS / HEAT PIPES
    // ----------------------------------------------------------------------

    const avionicsRadiatorTemp = 12.0 + thermalStress * 0.3 + wave(t, 0, 1.0, 53, 0.4);
    ...
    const avionicsRadiatorRejection = 72 + thermalStress * 2.3 + wave(t, 0, 4, 29, 0.4);
    ...
    const heatPipeAEvaporator = 34.0 + thermalStress * 0.55 + wave(t, 0, 0.6, 33, 0.5);

    const heatPipeACondenser = heatPipeAEvaporator - (2.2 + wave(t, 0, 0.25, 27, 0.8));

    const heatPipeBEvaporator =
      29.0 + payloadLoadFactor * 6 + thermalStress * 0.25 + wave(t, 0, 0.5, 36, 1.0);

    const heatPipeBCondenser = heatPipeBEvaporator - (1.8 + wave(t, 0, 0.2, 30, 1.2));

    // ----------------------------------------------------------------------
    // HEATERS / THERMOSTATS
    // ----------------------------------------------------------------------

    const avionicsHeaterDuty = clamp(
      8 - avionicsDeckTemp * 0.15 + (powerSavingModeActive.value ? 3 : 0),
      0,
      20
    );

    const payloadHeaterDuty = clamp(
      payloadOpticsTemp < 19 ? 35 : payloadOpticsTemp < 21 ? 15 : 0,
      0,
      100
    );

    const batteryHeaterDuty = clamp(
      batteryAverage < 15 ? 50 : batteryAverage < 20 ? 20 : 0,
      0,
      100
    );

    const survivalHeaterDuty = powerSavingModeActive.value ? 5 : 0;

    const avionicsHeaterCurrent = (avionicsHeaterDuty / 100) * 1.2;

    const payloadHeaterCurrent = (payloadHeaterDuty / 100) * 1.5;

    const batteryHeaterCurrent = (batteryHeaterDuty / 100) * 2.0;

    const survivalHeaterCurrent = (survivalHeaterDuty / 100) * 1.8;
    ...
    const externalHeatInput = 58 + Math.sin(t / 100) * 18;

    const totalHeatRejection = avionicsRadiatorRejection + payloadRadiatorRejection;

    const thermalBalance = estimatedDissipation + externalHeatInput - totalHeatRejection;
    ...
    const hottest = monitoredTemps.reduce((a, b) => (b.value > a.value ? b : a));

    const coldest = monitoredTemps.reduce((a, b) => (b.value < a.value ? b : a));

    const maximumGradient = hottest.value - coldest.value;

    // Simulator-only margins, not flight limits.
    const epsMargin = 75 - epsTemp;

    const payloadMargin = 55 - payloadElectronicsTemp;

    const batteryMargin = 45 - batteryAverage;

    const transmitterMargin = 65 - transmitterTemp;

    const simulatorThermalMargin = Math.min(
      epsMargin,
      payloadMargin,
      batteryMargin,
      transmitterMargin
    ...
    const thermalTrend = coolingActive
      ? 'COOLING'
      : thermalBalance > 20
        ? 'HEATING'
        : thermalBalance < -20
    ...
    const predictedEpsTemp = epsTemp + (coolingActive ? -2.0 : thermalStress > 0 ? 0.8 : 0.15);

    const predictedPayloadTemp =
      payloadElectronicsTemp + (coolingActive ? -1.5 : payloadHighPower ? 1.2 : 0.2);

    const predictedMargin = Math.min(
      75 - predictedEpsTemp,
    ...
    const thermalModelResidual = wave(t, 0.05, 0.08, 24, 0.7);
    ...
    return [
      ...
      row('TCS002', 'TCS Controller', 'ACTIVE', 'state'),
      ...
      row(
        ...
        '°C',
        temperatureStatus(controllerTemp, 15, 50, 0, 65)
      ),
      ...
      row('TCS005', 'TCS Overall Health', overallHealth, 'state', overallHealthStatus),
      ...
      row(
        ...
        epsTemp.toFixed(1),
        '°C',
        epsTemp <= 75 ? 'good' : epsTemp <= 85 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        pcuTemp.toFixed(1),
        '°C',
        temperatureStatus(pcuTemp, 15, 55, 0, 68)
      ),
      ...
      row(
        ...
        dcDcTemp.toFixed(1),
        '°C',
        temperatureStatus(dcDcTemp, 15, 58, 0, 70)
      ),
      ...
      row(
        ...
        pduTemp.toFixed(1),
        '°C',
        temperatureStatus(pduTemp, 10, 55, -5, 68)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(batteryAverage, 15, 35, 5, 45)
      ),
      ...
      row(
        ...
        batteryA.toFixed(1),
        '°C',
        temperatureStatus(batteryA, 15, 35, 5, 45)
      ),
      ...
      row(
        ...
        batteryB.toFixed(1),
        '°C',
        temperatureStatus(batteryB, 15, 35, 5, 45)
      ),
      ...
      row(
        ...
        batteryC.toFixed(1),
        '°C',
        temperatureStatus(batteryC, 15, 35, 5, 45)
      ),
      ...
      row(
        ...
        bmsTemp.toFixed(1),
        '°C',
        temperatureStatus(bmsTemp, 15, 42, 5, 52)
      ),
      ...
      row(
        ...
        obcTemp.toFixed(1),
        '°C',
        temperatureStatus(obcTemp, 10, 50, -5, 65)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(memoryTemp, 10, 48, -5, 62)
      ),
      ...
      row(
        ...
        commsTemp.toFixed(1),
        '°C',
        temperatureStatus(commsTemp, 10, 50, -5, 65)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(transmitterTemp, 15, 55, 0, 65)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(aocsComputerTemp, 10, 50, -5, 65)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(reactionWheelTemp, 5, 50, -10, 65)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(starTrackerTemp, -5, 40, -15, 50)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(payloadElectronicsTemp, 10, 45, 0, 55)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(payloadDetectorTemp, 5, 30, -5, 40)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(payloadOpticsTemp, 10, 30, 0, 40)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(focalPlaneTemp, 5, 28, -5, 38)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(payloadConverterTemp, 10, 50, 0, 62)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(opticalBenchTemp, 15, 28, 5, 35)
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(avionicsDeckTemp, 10, 45, 0, 55)
      ),

      // RADIATORS / HEAT PIPES
      row('RAD301', 'Avionics Radiator Temperature', avionicsRadiatorTemp.toFixed(1), '°C'),

      row('RAD302', 'Payload Radiator Temperature', payloadRadiatorTemp.toFixed(1), '°C'),

      row('RAD303', 'Avionics Radiator Heat Rejection', avionicsRadiatorRejection.toFixed(1), 'W'),

      row('RAD304', 'Payload Radiator Heat Rejection', payloadRadiatorRejection.toFixed(1), 'W'),

      row('HP310', 'Heat Pipe A Evaporator', heatPipeAEvaporator.toFixed(1), '°C'),

      row('HP311', 'Heat Pipe A Condenser', heatPipeACondenser.toFixed(1), '°C'),

      row(
        'HP312',
        'Heat Pipe A Temperature Drop',
        (heatPipeAEvaporator - heatPipeACondenser).toFixed(2),
        '°C'
      ),

      row('HP313', 'Heat Pipe A State', 'OPERATING', 'state'),

      row('HP320', 'Heat Pipe B Evaporator', heatPipeBEvaporator.toFixed(1), '°C'),

      row('HP321', 'Heat Pipe B Condenser', heatPipeBCondenser.toFixed(1), '°C'),

      row(
        'HP322',
        'Heat Pipe B Temperature Drop',
        (heatPipeBEvaporator - heatPipeBCondenser).toFixed(2),
        '°C'
      ),

      row('HP323', 'Heat Pipe B State', 'OPERATING', 'state'),
      ...
      // HEATERS
      row('HTR401', 'Heater Controller', 'ACTIVE', 'state'),

      row(
        'HTR410',
        'Avionics Operational Heater Command',
        avionicsHeaterDuty > 0 ? 'ON' : 'OFF',
        'state'
      ),

      row('HTR411', 'Avionics Operational Heater Duty Cycle', avionicsHeaterDuty.toFixed(1), '%'),

      row('HTR412', 'Avionics Operational Heater Current', avionicsHeaterCurrent.toFixed(3), 'A'),

      row('HTR420', 'Payload Heater Command', payloadHeaterDuty > 0 ? 'ON' : 'OFF', 'state'),

      row('HTR421', 'Payload Heater Duty Cycle', payloadHeaterDuty.toFixed(1), '%'),

      row('HTR422', 'Payload Heater Current', payloadHeaterCurrent.toFixed(3), 'A'),

      row('HTR430', 'Battery Heater Command', batteryHeaterDuty > 0 ? 'ON' : 'OFF', 'state'),

      row('HTR431', 'Battery Heater Duty Cycle', batteryHeaterDuty.toFixed(1), '%'),

      row('HTR432', 'Battery Heater Current', batteryHeaterCurrent.toFixed(3), 'A'),

      row(
        'HTR440',
        'Survival Heater Bus',
        powerSavingModeActive.value ? 'ARMED' : 'STANDBY',
        'state'
      ),

      row('HTR441', 'Survival Heater Duty Cycle', survivalHeaterDuty.toFixed(1), '%'),

      row('HTR442', 'Survival Heater Current', survivalHeaterCurrent.toFixed(3), 'A'),

      row('HTR450', 'Decontamination Heater', 'NOT CONFIGURED', 'state', 'empty'),

      row('HTR451', 'Propellant Tank Heater', 'NOT CONFIGURED', 'state', 'empty'),

      row('HTR452', 'Thruster Catalyst Heater', 'NOT CONFIGURED', 'state', 'empty'),

      // THERMOSTATS
      row(
        'THM460',
        'Avionics Thermostat',
        avionicsHeaterDuty > 0 ? 'CALL FOR HEAT' : 'SATISFIED',
        'state'
      ),
      ...
      row(
        'THM461',
        'Payload Thermostat',
        payloadHeaterDuty > 0 ? 'CALL FOR HEAT' : 'SATISFIED',
        'state'
      ),
      ...
      row(
        'THM462',
        'Battery Thermostat',
        batteryHeaterDuty > 0 ? 'CALL FOR HEAT' : 'SATISFIED',
        'state'
      ),
      ...
      row('SNS505', 'Thermistor Reference Error', wave(t, 0.03, 0.02, 43, 0.3).toFixed(3), '°C'),

      row('SNS506', 'RTD Reference Error', wave(t, 0.02, 0.015, 47, 0.8).toFixed(3), '°C'),

      row('SNS507', 'Thermocouple Reference Error', wave(t, 0.08, 0.04, 51, 1.1).toFixed(3), '°C'),

      // THERMAL BALANCE
      row('THB601', 'Estimated Spacecraft Dissipation', estimatedDissipation.toFixed(1), 'W'),

      row('THB602', 'Estimated External Heat Input', externalHeatInput.toFixed(1), 'W'),

      row('THB603', 'Estimated Total Heat Rejection', totalHeatRejection.toFixed(1), 'W'),
      ...
      row(
        ...
        'W',
        Math.abs(thermalBalance) <= 35 ? 'good' : Math.abs(thermalBalance) <= 70 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '°C',
        maximumGradient <= 45 ? 'good' : maximumGradient <= 60 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '°C',
        simulatorThermalMargin >= 5 ? 'good' : simulatorThermalMargin >= 0 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        'state',
        simulatorThermalMargin >= 0 ? 'good' : 'bad'
      ),
      ...
      row(
        ...
        thermalTrend,
        'state',
        thermalTrend === 'STABLE' ? 'good' : 'warning'
      ),

      // PREDICTION
      row('EST701', 'Thermal State Estimator', 'VALID', 'state'),
      ...
      row(
        ...
        '°C',
        predictedEpsTemp <= 75 ? 'good' : predictedEpsTemp <= 85 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(predictedPayloadTemp, 10, 45, 0, 55)
      ),
      ...
      row(
        ...
        '°C',
        predictedMargin >= 5 ? 'good' : predictedMargin >= 0 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '°C',
        Math.abs(thermalModelResidual) <= 0.5 ? 'good' : 'warning'
      ),
