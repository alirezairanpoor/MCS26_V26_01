...
export type PayloadTelemetryStatus = 'empty' | 'good' | 'warning' | 'bad';
...
function noTelemetryRows(definitions: TelemetryDefinition[]): PayloadTelemetryRow[] {
...
function temperatureStatus(
  ...
  if (value < nominalLow || value > nominalHigh) {
    return 'warning';
  }
...
export function usePayloadTelemetry(options: UsePayloadTelemetryOptions) {
  const {
    missionSeconds,
  ...
  const payloadTelemetry = computed<PayloadTelemetryRow[]>(() => {
    if (!spacecraftTelemetryAvailable.value || scenario2TelemetryBlackout.value) {
      return noTelemetryRows(payloadTelemetryDefinition);
    }
    ...
    const scenario1FrankfurtWindow = !isScenario2.value && t >= 900 && t <= 960;
    ...
    const imagingWindowActive =
      scenario1FrankfurtWindow || scenario2CalibrationWindow || scenario2FrankfurtWindow;

    const targetName =
      scenario2CalibrationWindow || (isScenario2.value && !scenario2NewProcedureImported.value)
        ? 'CALIBRATION TEST'
        : 'FRANKFURT AIRPORT';

    const acquisitionType =
      isScenario2.value && !scenario2NewProcedureImported.value
        ? 'CALIBRATION IMAGE'
        : 'TARGET IMAGE';
    ...
    const powerLevel = payloadPowerLevel.value;

    const payloadActive = payloadPowerRaised.value && !spacecraftStandbyActive.value;

    const warming = powerIncreaseInProgress.value;

    const cooling = thermalCoolingActive.value;

    const powerSaving = powerSavingModeActive.value;

    const configured = cameraConfigured.value;

    const memoryUse = clamp(memoryUsed.value, 0, 100);

    const memoryAvailable = 100 - memoryUse;
    ...
    const instrumentMode = payloadActive ? 'ACTIVE' : 'STANDBY';

    let operationalState = 'PAYLOAD_STANDBY';

    if (spacecraftStandbyActive.value) {
      operationalState = 'PAYLOAD_STANDBY';
    } else if (cooling) {
      operationalState = 'PAYLOAD_COOLDOWN';
    } else if (warming) {
      operationalState = 'PAYLOAD_WARMUP';
    } else if (payloadActive && configured && scenario2CalibrationWindow) {
      operationalState = 'PAYLOAD_CALIBRATION';
    } else if (payloadActive && configured && imagingWindowActive) {
      operationalState = 'PAYLOAD_ACQUISITION';
    } else if (payloadActive && configured) {
      operationalState = 'PAYLOAD_READY';
    } else if (payloadActive) {
      operationalState = 'PAYLOAD_ACTIVE';
    }

    // Generic simulator-only thermal readiness.
    const thermalReady = epsTemperature.value < 90;

    const storageReady = memoryUse < 95;
    ...
    const payloadHealthStatus: PayloadTelemetryStatus =
      epsTemperature.value >= 95
        ? 'bad'
        : memoryUse >= 95 || epsTemperature.value >= 90
          ? 'warning'
          : 'good';
    ...
    const busVoltage = 28.0 + wave(t, 0, 0.1, 21, 0.4);

    const busCurrent = powerLevel / Math.max(busVoltage, 1);

    const converterEfficiency = clamp(
      92.5 - Math.abs(powerLevel - 90) * 0.008 + wave(t, 0, 0.25, 27, 0.8),
      88,
      95
    );
    ...
    const detectorSupply = configured ? wave(t, 5.1, 0.035, 18, 0.4) : wave(t, 5.02, 0.02, 21, 0.9);

    const detectorBias = configured ? wave(t, 38.0, 1.2, 24, 0.5) : 0;

    const detectorBiasCurrent = configured ? wave(t, 18.0, 1.5, 22, 0.7) : 0;

    const analogRail = wave(t, 3.3, 0.018, 19, 0.3);

    const digitalRail = wave(t, 1.8, 0.012, 23, 1.0);
    ...
    const fpgaLoad = clamp(
      18 + (configured ? 20 : 0) + (imagingWindowActive ? 18 : 0) + wave(t, 0, 3, 17, 0.9),
      8,
      90
    );

    const cpuLoad = clamp(
      22 + (imageTaken.value ? 8 : 0) + (configured ? 8 : 0) + wave(t, 0, 4, 20, 1.1),
      8,
      85
    );
    ...
    const fpgaTemp = controllerTemp + 2.0 + fpgaLoad * 0.025;

    const cpuTemp = controllerTemp + 1.0 + cpuLoad * 0.02;

    // ----------------------------------------------------------------------
    // OPTICS / DETECTOR
    // ----------------------------------------------------------------------

    const baffleTemp = wave(t, -12.0, 5.0, 70, 0.8);

    const lensTemp = 8.0 + powerLevel * 0.015 - (cooling ? 0.8 : 0) + wave(t, 0, 2.0, 58, 2.4);

    const opticalBenchTemp = 19.0 + powerLevel * 0.018 + wave(t, 0, 0.45, 61, 0.6);
    ...
    const focalPlaneTemp = sensorTemp - 1.8 + wave(t, 0, 0.25, 39, 0.2);

    const adcTemp = 28.0 + powerLevel * 0.03 + wave(t, 0, 0.7, 37, 1.1);
    ...
    const focusPosition = configured ? Math.round(wave(t, 2048, 2, 80, 0.4)) : 2048;

    const focusError = configured ? Math.abs(wave(t, 0, 1.2, 30, 0.7)) : 4.0;

    const alignmentError = Math.abs(wave(t, 0, 3.5, 55, 0.9));

    const throughput = clamp(wave(t, 91.5, 0.7, 63, 0.5), 85, 95);

    const contamination = clamp(wave(t, 0.8, 0.12, 120, 0.4), 0, 2);

    const readNoise = wave(t, 4.2, 0.25, 34, 0.6);

    const darkCurrent = clamp(
      0.75 + Math.max(0, sensorTemp - 12) * 0.055 + wave(t, 0, 0.05, 48, 0.8),
      0.4,
      2.5
    );

    const saturationFraction =
      imagingWindowActive && configured ? clamp(wave(t, 0.8, 0.25, 19, 0.4), 0, 2) : 0;

    const badPixelFraction = clamp(wave(t, 0.18, 0.025, 90, 0.7), 0.1, 0.3);

    // ----------------------------------------------------------------------
    // TIMING / GNSS
    // ----------------------------------------------------------------------

    const gpsTemp = wave(t, 31.0, 1.1, 47, 1.4);

    const gnssSatellites = Math.round(clamp(wave(t, 10, 2, 31, 0.5), 7, 13));

    const timingOffset = wave(t, 0, 0.2, 18, 0.7);

    // ----------------------------------------------------------------------
    // CALIBRATION
    // ----------------------------------------------------------------------

    const calibrationActive = scenario2CalibrationWindow && configured && payloadActive;

    const calibrationAge = Math.max(0, Math.round(7200 + t));

    const calibrationResidual = wave(t, 0.7, 0.12, 44, 0.6);

    // ----------------------------------------------------------------------
    // IMAGE / QUALITY
    // ----------------------------------------------------------------------

    const exposureTime = acquisitionType === 'CALIBRATION IMAGE' ? 2.5 : 3.8;

    const sensorGain = acquisitionType === 'CALIBRATION IMAGE' ? 5.0 : 7.5;

    const imageSnr = configured
      ? clamp(38 + throughput * 0.15 - readNoise * 0.8 + wave(t, 0, 1.4, 25, 0.8), 25, 60)
      : 0;

    const meanSignal =
      configured && imagingWindowActive ? clamp(wave(t, 2120, 180, 18, 0.4), 500, 3900) : 0;

    const contrast = configured && imagingWindowActive ? clamp(wave(t, 48, 4, 23, 0.9), 30, 65) : 0;

    // ----------------------------------------------------------------------
    // COMPRESSION / DATA
    // ----------------------------------------------------------------------

    const compressionQueue = clamp(
      memoryUse * 0.1 + (imageTaken.value ? 8 : 2) + wave(t, 0, 2, 17, 1.2),
      0,
      90
    );

    const compressionRatio = 3.2 + wave(t, 0, 0.15, 29, 0.5);

    const compressionThroughput = configured ? wave(t, 92, 6, 26, 0.8) : wave(t, 15, 2, 31, 0.4);

    const compressedImageSize = 42 / Math.max(compressionRatio, 1);

    const dataBuffer = clamp(
      memoryUse * 0.12 + (imageTaken.value ? 6 : 0) + wave(t, 0, 1.5, 20, 0.7),
      0,
      95
    );

    const outputQueue = clamp(
      memoryUse * 0.08 + (imageTaken.value ? 5 : 1) + wave(t, 0, 1.3, 22, 1.1),
      0,
      90
    );
    ...
    const electronicsThermalMargin = 55 - controllerTemp;

    const detectorThermalMargin = 35 - sensorTemp;

    const payloadThermalTrend = cooling
      ? 'COOLING'
      : warming || powerLevel > 120
        ? 'HEATING'
        : 'STABLE';

    const heaterDuty = sensorTemp < 8 ? 35 : sensorTemp < 11 ? 12 : 0;
    ...
    return [
      // SYSTEM / MODES
      row('PLD600', 'Payload Overall Health', payloadHealth, 'state', payloadHealthStatus),

      row(
        'PLD601',
        'Payload Electronics Unit',
        spacecraftStandbyActive.value ? 'STANDBY' : 'ACTIVE',
        'state'
      ),
      ...
      row('PLD603', 'Payload Data Processing Unit', payloadActive ? 'ACTIVE' : 'STANDBY', 'state'),

      row(
        'PLD604',
        'Payload Command Queue',
        powerIncreaseInProgress.value ? '1' : '0',
        'count',
        powerIncreaseInProgress.value ? 'warning' : 'good'
      ),

      row('PLD605', 'Payload Fault Counter', '0', 'count'),

      row('PLD606', 'Payload Watchdog', 'NOMINAL', 'state'),

      row(
        'PLD607',
        'Payload FDIR State',
        payloadHealthStatus === 'bad' ? 'MONITORING FAULT' : 'NOMINAL',
        'state',
        payloadHealthStatus
      ),

      // Preserve the simulator/procedure-facing PLD620 behavior.
      row('PLD620', 'Instrument Mode', instrumentMode, 'state', 'good'),
      ...
      row(
        ...
        operationalState,
        'state',
        operationalState === 'PAYLOAD_COOLDOWN' ? 'warning' : 'good'
      ),

      row(
        'PLD622',
        'Acquisition Readiness',
        acquisitionReady ? 'READY' : 'NOT READY',
        'state',
        acquisitionReady ? 'good' : 'warning'
      ),

      row(
        'PLD623',
        'Payload Thermal Readiness',
        thermalReady ? 'READY' : 'NOT READY',
        'state',
        thermalReady ? 'good' : 'bad'
      ),
      ...
      row(
        ...
        'V',
        busVoltage >= 27.5 && busVoltage <= 28.5 ? 'good' : 'warning'
      ),
      ...
      row(
        ...
        'A',
        busCurrent <= 7.5 ? 'good' : busCurrent <= 9 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(converterTemp, 10, 50, 0, 62)
      ),
      ...
      row(
        ...
        '%',
        converterEfficiency >= 90 ? 'good' : 'warning'
      ),
      ...
      row(
        ...
        'V',
        detectorSupply >= 4.9 && detectorSupply <= 5.3 ? 'good' : 'warning'
      ),

      row('PLD741', 'Detector Bias', detectorBias.toFixed(1), 'V', configured ? 'good' : 'empty'),
      ...
      row(
        ...
        'mA',
        configured ? 'good' : 'empty'
      ),

      row('PLD743', 'Analog Rail', analogRail.toFixed(3), 'V', 'good'),

      row('PLD744', 'Digital Rail', digitalRail.toFixed(3), 'V', 'good'),
      ...
      row(
        ...
        '°C',
        temperatureStatus(controllerTemp, 15, 52, 5, 65)
      ),
      ...
      row(
        ...
        '%',
        controllerLoad <= 75 ? 'good' : controllerLoad <= 90 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        fpgaTemp.toFixed(1),
        '°C',
        temperatureStatus(fpgaTemp, 15, 55, 5, 68)
      ),
      ...
      row(
        ...
        fpgaLoad.toFixed(0),
        '%',
        fpgaLoad <= 75 ? 'good' : fpgaLoad <= 90 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        cpuTemp.toFixed(1),
        '°C',
        temperatureStatus(cpuTemp, 15, 55, 5, 68)
      ),
      ...
      row(
        ...
        cpuLoad.toFixed(0),
        '%',
        cpuLoad <= 75 ? 'good' : cpuLoad <= 90 ? 'warning' : 'bad'
      ),

      row('PLD555', 'Payload Software State', 'RUNNING', 'state'),

      row('PLD556', 'Payload Uptime', t.toFixed(0), 's'),
      ...
      row(
        ...
        '°C',
        temperatureStatus(cameraHeadTemp, 10, 35, -5, 48)
      ),

      row(
        'CAM000',
        'Camera Configuration',
        configured ? 'READY' : 'STANDBY',
        'state',
        configured ? 'good' : 'warning'
      ),

      row('CAM202', 'Camera Electronics', payloadActive ? 'ACTIVE' : 'STANDBY', 'state'),

      row(
        'CAM203',
        'Camera Clock',
        configured ? wave(t, 48.0, 0.02, 50, 0.4).toFixed(3) : '0.000',
        'MHz',
        configured ? 'good' : 'empty'
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(baffleTemp, -25, 5, -45, 25)
      ),

      row('OBF045', 'Optical Baffle State', 'NOMINAL', 'state'),
      ...
      row(
        ...
        lensTemp.toFixed(1),
        '°C',
        temperatureStatus(lensTemp, -5, 25, -20, 40)
      ),

      row('LNS055', 'Lens Focus Position', focusPosition.toString(), 'step'),
      ...
      row(
        ...
        'µm',
        focusError <= 2 ? 'good' : focusError <= 5 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(opticalBenchTemp, 12, 28, 5, 35)
      ),
      ...
      row(
        ...
        '%',
        contamination <= 2 ? 'good' : 'warning'
      ),

      row('OPT105', 'Aperture State', 'OPEN / FIXED', 'state'),
      ...
      row(
        ...
        '°C',
        temperatureStatus(sensorTemp, -2, 22, -15, 35)
      ),
      ...
      row(
        ...
        '%',
        saturationFraction <= 2 ? 'good' : saturationFraction <= 5 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '%',
        badPixelFraction <= 0.5 ? 'good' : 'warning'
      ),

      row(
        'SEN337',
        'Detector Gain',
        configured ? '1.80' : '0.00',
        'e-/DN',
        configured ? 'good' : 'empty'
      ),

      row('SEN338', 'Detector Bit Depth', '12', 'bit'),
      ...
      row(
        ...
        '°C',
        temperatureStatus(focalPlaneTemp, -5, 20, -15, 30)
      ),
      ...
      row(
        'FPA343',
        'Focal Plane Clock',
        configured ? '24.000' : '0.000',
        'MHz',
        configured ? 'good' : 'empty'
      ),

      row('FPA344', 'ADC Reference Voltage', wave(t, 2.5, 0.004, 37, 0.6).toFixed(3), 'V'),
      ...
      row(
        ...
        adcTemp.toFixed(1),
        '°C',
        temperatureStatus(adcTemp, 10, 45, 0, 58)
      ),
      ...
      row(
        ...
        gpsTemp.toFixed(1),
        '°C',
        temperatureStatus(gpsTemp, 20, 45, 5, 58)
      ),

      row('GPS113', 'PPS Lock', 'LOCK', 'state'),

      row('GPS114', 'Payload GNSS Fix', '3D FIX', 'state'),
      ...
      row(
        ...
        'µs',
        Math.abs(timingOffset) <= 1 ? 'good' : 'warning'
      ),

      row('GPS117', 'Image Timestamp Validity', 'VALID', 'state'),

      // CALIBRATION
      row(
        'CAL401',
        'Calibration Unit',
        calibrationActive ? 'ACTIVE' : 'STANDBY',
        'state',
        calibrationActive ? 'good' : 'empty'
      ),

      row('CAL402', 'Dark Reference State', calibrationActive ? 'VALID' : 'AVAILABLE', 'state'),

      row(
        'CAL403',
        'Flat Field Calibration State',
        calibrationActive ? 'RUNNING' : 'VALID',
        'state',
        calibrationActive ? 'warning' : 'good'
      ),

      row('CAL404', 'Calibration Validity', 'VALID', 'state'),

      row('CAL405', 'Calibration Age', calibrationAge.toString(), 's'),
      ...
      row(
        ...
        '%',
        calibrationResidual <= 2 ? 'good' : 'warning'
      ),
      ...
      row(
        'IMG901',
        'Image Capture',
        imageTaken.value ? imageValidity.value : 'NO IMAGE',
        'state',
        imageTaken.value ? 'good' : 'empty'
      ),

      row(
        'IMG902',
        'Imaging Window State',
        imagingWindowActive ? 'IN WINDOW' : 'OUTSIDE WINDOW',
        'state',
        imagingWindowActive ? 'good' : 'empty'
      ),

      row('IMG903', 'Active Imaging Target', targetName, 'state'),

      row('IMG904', 'Acquisition Type', acquisitionType, 'state'),

      row(
        'IMG905',
        'Exposure Time',
        configured ? exposureTime.toFixed(2) : '0.00',
        'ms',
        configured ? 'good' : 'empty'
      ),
      ...
      row(
        'IMG906',
        'Sensor Gain',
        configured ? sensorGain.toFixed(1) : '0.0',
        'dB',
        configured ? 'good' : 'empty'
      ),

      row('IMG907', 'Image Width', '4096', 'px'),

      row('IMG908', 'Image Height', '3072', 'px'),

      row('IMG909', 'Image Sequence Counter', imageTaken.value ? '1' : '0', 'count'),

      row(
        'IMG910',
        'Last Image File',
        imageTaken.value ? capturedImageName.value || 'IMAGE_CAPTURED' : 'NONE',
        'state',
        imageTaken.value ? 'good' : 'empty'
      ),
      ...
      row(
        ...
        imageTaken.value ? imageValidity.value : 'NO IMAGE',
        'state',
        imageTaken.value ? (imageValidity.value.includes('VALID') ? 'good' : 'warning') : 'empty'
      ),

      row(
        'IMG912',
        'Image Signal-to-Noise Ratio',
        configured ? imageSnr.toFixed(1) : 'NO DATA',
        'dB',
        configured ? (imageSnr >= 35 ? 'good' : imageSnr >= 25 ? 'warning' : 'bad') : 'empty'
      ),

      row(
        'IMG913',
        'Image Mean Signal',
        configured && imagingWindowActive ? meanSignal.toFixed(0) : 'NO DATA',
        'DN',
        configured && imagingWindowActive ? 'good' : 'empty'
      ),

      row(
        'IMG914',
        'Image Contrast Estimate',
        configured && imagingWindowActive ? contrast.toFixed(1) : 'NO DATA',
        '%',
        configured && imagingWindowActive ? 'good' : 'empty'
      ),
      ...
      row(
        'IMG915',
        'Image CRC',
        imageTaken.value ? 'VALID' : 'NO DATA',
        'state',
        imageTaken.value ? 'good' : 'empty'
      ),
      ...
      row(
        'IMG916',
        'Image Metadata',
        imageTaken.value ? 'COMPLETE' : 'NO DATA',
        'state',
        imageTaken.value ? 'good' : 'empty'
      ),
      ...
      row(
        ...
        (controllerTemp + 3.0).toFixed(1),
        '°C',
        temperatureStatus(controllerTemp + 3.0, 20, 55, 10, 68)
      ),
      ...
      row(
        ...
        '%',
        compressionQueue <= 70 ? 'good' : compressionQueue <= 90 ? 'warning' : 'bad'
      ),

      row('CMP553', 'Compression Processor State', payloadActive ? 'ACTIVE' : 'STANDBY', 'state'),

      row('CMP554', 'Compression Mode', 'LOSSLESS + PREDICTIVE', 'state'),

      row('CMP555', 'Compression Ratio', compressionRatio.toFixed(2), ':1'),

      row('CMP556', 'Compression Throughput', compressionThroughput.toFixed(1), 'Mbps'),

      row('CMP557', 'Compressed Image Size', compressedImageSize.toFixed(1), 'MB'),

      row('CMP558', 'Compression Error Counter', '0', 'count'),

      // DATA / MEMORY INTERFACE
      row('DAT601', 'Payload Mass Memory Interface', 'ONLINE', 'state'),
      ...
      row(
        ...
        memoryUse.toFixed(0),
        '%',
        memoryUse <= 70 ? 'good' : memoryUse <= 90 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '%',
        memoryAvailable >= 30 ? 'good' : memoryAvailable >= 10 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '%',
        dataBuffer <= 70 ? 'good' : dataBuffer <= 90 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '%',
        outputQueue <= 70 ? 'good' : outputQueue <= 90 ? 'warning' : 'bad'
      ),

      row('DAT606', 'Payload File System', 'MOUNTED / RW', 'state'),

      row('DAT607', 'Payload Data Integrity', 'VALID', 'state'),

      row(
        'DAT608',
        'Payload Recorder Interface Rate',
        payloadActive ? wave(t, 145, 8, 25, 0.4).toFixed(1) : '0.0',
        'Mbps',
        payloadActive ? 'good' : 'empty'
      ),
      ...
      row(
        ...
        '°C',
        temperatureStatus(radiatorTemp, -35, 0, -55, 22)
      ),

      row(
        'RAD088',
        'Payload Radiator State',
        cooling ? 'COOLDOWN SUPPORT' : 'NOMINAL',
        'state',
        cooling ? 'warning' : 'good'
      ),
      ...
      row(
        ...
        '°C',
        electronicsThermalMargin >= 5 ? 'good' : electronicsThermalMargin >= 0 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        '°C',
        detectorThermalMargin >= 5 ? 'good' : detectorThermalMargin >= 0 ? 'warning' : 'bad'
      ),
      ...
      row(
        ...
        payloadThermalTrend,
        'state',
        payloadThermalTrend === 'STABLE' ? 'good' : 'warning'
      ),

      row('THM704', 'Payload Heater State', heaterDuty > 0 ? 'ON' : 'OFF', 'state'),

      row('THM705', 'Payload Heater Duty Cycle', heaterDuty.toFixed(0), '%'),
      ...
      row('IFC802', 'AOCS Rate Interface', 'VALID', 'state'),

      row(
        'IFC803',
        'EPS Power Allocation',
        payloadActive ? 'CONFIRMED' : warming ? 'RAMPING' : 'STANDBY',
        'state',
        payloadActive ? 'good' : warming ? 'warning' : 'empty'
      ),

      row(
        'IFC804',
        'TCS Thermal Interface',
        thermalReady ? 'READY' : 'THERMAL INHIBIT',
        'state',
        thermalReady ? 'good' : 'bad'
      ),
      ...
      row(
        ...
        storageReady ? 'READY' : 'CAPACITY INHIBIT',
        'state',
        storageReady ? 'good' : 'bad'
      ),

      row('IFC806', 'Spacecraft Time Interface', 'VALID', 'state'),

      // OPTIONAL / NON-BASELINE HARDWARE
      row('MEC901', 'Payload Cover Mechanism', 'NOT INSTALLED', 'state', 'empty'),

      row('MEC902', 'Payload Door Mechanism', 'NOT INSTALLED', 'state', 'empty'),

      row('MEC903', 'Variable Focus Mechanism', 'NOT INSTALLED / FIXED FOCUS', 'state', 'empty'),

      row('ANT910', 'Payload Dedicated Antenna', 'NOT INSTALLED', 'state', 'empty'),
    ];
  });
